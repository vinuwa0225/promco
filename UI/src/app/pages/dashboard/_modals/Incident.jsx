...
function Incident() {
  ...
  useEffect(() => {
    const getddtdata = axios({
      ...axioDefaultConfig,
      method: 'get',
      url: process.env.REACT_APP_INSEE_KIOSK_BACKEND_API + '/api/report/incidentrecord',
...
*/
            ...
            getddtdataResponse?.data?.result.map((item) => {
              data.push({name: item.month, month: item.count})
            })
            setState(data)
    ...
    <div className='card card-xl-stretch mb-xl-8'>
      ...
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bolder text-dark'>Incidents</span>
          <span className='text-muted mt-1 fw-bold fs-7'>Last 12 Months</span>
        </h3>
      ...
      <div className='card-body pt-2'>
        ...
        {!InitialLoading && (
          <ResponsiveContainer width='100%' height={300}>
            <LineChart width={300} height={100} data={state}>
              <Line type='monotone' dataKey='month' stroke='#8884d8' strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
