...
function CSMDStatus() {
  ...
  useEffect(() => {
    const getddtdata = axios({
      ...axioDefaultConfig,
      method: 'get',
      url: process.env.REACT_APP_INSEE_KIOSK_BACKEND_API + '/api/report/csmdstatus',
...
*/
            ...
            getddtdataResponse?.data?.result.map((item) => {
              data.push({name: item.csmdStatus, value: item.count})
            })
            setState(data)
    ...
    <div className='card card-xl-stretch mb-xl-8'>
      ...
      <div className='card-header border-0 pt-5'>
        <h3 className='card-title align-items-start flex-column'>
          <span className='card-label fw-bolder text-dark'>CSMD Status</span>
